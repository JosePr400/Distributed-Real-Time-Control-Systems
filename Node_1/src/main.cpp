...
void setup() {
    ...
    G[1] = (L2 - L1)/DAC_RANGE; // Calculate system Gain with to points, the maximum and the minimum
    ...
    pi_control.set_coefficients(0.01,1,0.3,0.08,sqrt(0.08),G[1]);
...
void loop() { // Core 0
    ...
    if (current_time - previous_control_time >= CONTROL_INTERVAL) {
        ...
        v_ldr[1] = sensorValue * (3.3 / 4095.0);
        float y_raw = convertToLux(sensorValue);
                    
        // Apply exponential filter
        y_filtered[1] = ALPHA * y_raw + (1 - ALPHA) * y_filtered[1];
        ...
        if(is_active[1] == 0){
            ...
            if(manual[1] == 0) {
                ...
                u[1] = pi_control.compute_control(reference[1], y_filtered[1]);

                pwm[1] = (int)u[1];   
                d[1] = y_filtered[1] - G[1]*pwm[1];    
            }
        ...
        else if (is_active[1] == 1) {

            pi_control.turn_feedback(is_active[1]);
            pi_control.set_anti_windup(anti_windup[1]); 
            u[1] = pi_control.compute_control(reference[1], y_raw);
            pwm[1] = (int)u[1];
        ...
        analogWrite(LED_PIN, pwm[1]);

        pi_control.housekeep(reference[1], y_raw);

        // Update metrics
        system_metrics.update(pwm[1]/(DAC_RANGE-1.0f), current_time, reference[1], y_filtered[1]);
...
void sendAllValues() {
    ...
    while (true) {
        ...
        if (millis() - lastSendAll >= 1000) {
            ...
            sendBuffer(CANID_BUFF(1),data_buffer);
