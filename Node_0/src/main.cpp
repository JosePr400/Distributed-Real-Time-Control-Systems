...
// System components
pi pi_control(0.01,1,1,1,1,1,1);
Metrics system_metrics(P_MAX);
MCP2515 can0(spi0, 17, 19, 16, 18, 10000000);

// Data logging
Sample data_buffer[BUFFER_SIZE];
int write_index = 0;
...
void setup() {
    ...
    G[0] = (L2 - L1)/DAC_RANGE; // Calculate system Gain with to points, the maximum and the minimum
    ...
    pi_control.set_coefficients(0.01,1,0.3,0.08,sqrt(0.08),G[0]);
    
    multicore_launch_core1(readAllValues); // Using Core 1 
...
void loop() { // Core 0
    ...
    if (current_time - previous_control_time >= CONTROL_INTERVAL) {
        ...
        v_ldr[0] = sensorValue * (3.3 / 4095.0);
        float y_raw = convertToLux(sensorValue);
                    
        // Apply exponential filter
        y_filtered[0] = ALPHA * y_raw + (1 - ALPHA) * y_filtered[0];
        ...
        if(is_active[0] == 0){
            ...
            if(manual[0] == 0) {
                ...
                pi_control.turn_feedback(is_active[0]);
                pi_control.set_anti_windup(anti_windup[0]); 
                u[0] = pi_control.compute_control(reference[0], y_filtered[0]);

                pwm[0] = (int)u[0];   
                d[0] = y_filtered[0] - G[0]*pwm[0];    
            }
        ...
        else if (is_active[0] == 1) {

            pi_control.turn_feedback(is_active[0]);
            pi_control.set_anti_windup(anti_windup[0]); 
            u[0] = pi_control.compute_control(reference[0], y_raw);
            pwm[0] = (int)u[0];
        ...
        analogWrite(LED_PIN, pwm[0]);

        pi_control.housekeep(reference[0], y_raw);

        // Update metrics
        system_metrics.update(pwm[0]/(DAC_RANGE-1.0f), current_time, reference[0], y_filtered[0]);
    ...
    if(write_index == 0) buffer_full = true;
    interrupts();
...
void Hub(String input) {
    ...
    // Get instantaneous power consumption of desk 'g p <i>'
        ...
        int i = input.substring(4).toInt();
        if (i == 0) {
            power[0] = P_MAX * (pwm[0]/(DAC_RANGE-1.0f));
            Serial.print("p 0 ");
            Serial.print(power[0], 3);
            Serial.println(" [W]");
        }
        else if(i == 1){
            // Reiceive values
            power[1] = P_MAX * (pwm[1]/(DAC_RANGE-1.0f));
            Serial.print("p 1 ");
            Serial.print(power[1], 3);
            Serial.println(" [W]");
        }
        else if(i == 2){
            // Reiceive values
            power[2] = P_MAX * (pwm[1]/(DAC_RANGE-1.0f));
            Serial.print("p 2 ");
            Serial.print(power[2], 3);
            Serial.println(" [W]");
        }
        else {
            Serial.println("err");
    ...
    // Get the last minute buffer of the variable <x> of the desk 'g b <x> <i>'
        ...
        for(int count = 0; count < num_elements; count++) {
            ...
            if(var == "y") {
                Serial.print(data_buffer[index].y, 3);  // 3 decimal places for illuminance
            } else {
                Serial.print(data_buffer[index].u);   // Integer PWM values
            }
...
void sendBuffer(uint32_t canId, Sample data_buffer[BUFFER_SIZE]) {
    for (int i = 0; i < BUFFER_SIZE; i++) {
        // Frame 1: time_sec + r
        {
            ...
            frame.can_id  = canId;
            frame.can_dlc = 8;
            memcpy(&frame.data[0], &data_buffer[i].time_sec, 4);
            memcpy(&frame.data[4], &data_buffer[i].r,         4);
            can0.sendMessage(&frame);
        }
        // Frame 2: y + u
        {
        ...
        // Frame 3: pwm (plus padding)
        {
            struct can_frame frame;
            frame.can_id  = canId;
            frame.can_dlc = 8;
            memcpy(&frame.data[0], &data_buffer[i].pwm, 4);
...
void sendAllValues() {
    ...
    while (true) {
        ...
        if (millis() - lastSendAll >= 1000) {
            lastSendAll = millis();
            // Send values for node 1 and 2
            sendFloat(CANID_U(0), u[0],u[1],u[2]);
            sendFloat(CANID_REF(0), reference[0],reference[1],reference[2]);
            sendFloat(CANID_Y(0), y_filtered[0],y_filtered[1],y_filtered[2]);
            sendFloat(CANID_V(0),v_ldr[0],v_ldr[1],v_ldr[2]);
            sendBool(CANID_A(0),anti_windup[0],anti_windup[1],anti_windup[2]);
            sendBool(CANID_OCC(0),is_occupied[0],is_occupied[1],is_occupied[2]);
            sendBool(CANID_F(0),is_active[0],is_active[1],is_active[2]);
            sendFloat(CANID_D(0),d[0],d[1],d[2]);
            sendFloat(CANID_P(0),power[0],power[1],power[2]);
            sendFloat(CANID_T(0),elapsed[0],elapsed[1],elapsed[2]);
            sendBuffer(CANID_BUFF(0),data_buffer);
            sendFloat(CANID_ENERGY(0),E[0],E[1],E[2]);
            sendFloat(CANID_VIS(0),V[0],V[1],V[2]);
            sendFloat(CANID_FLICK(0),F[0],F[1],F[2]);
            sendBool(CANID_MAN(0),manual[0],manual[1],manual[2]);
    ...
    while (can0.readMessage(&rx) == MCP2515::ERROR_OK)
    {
        ...
        case CANID_BUFF(1):
            if (rx.can_dlc == 8) {
                unsigned long tsec;
                float yf;
                memcpy(&tsec, &rx.data[0], 4);
                memcpy(&yf,   &rx.data[4], 4);
            }
            break;
